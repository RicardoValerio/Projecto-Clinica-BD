  ...
  if (!empty($_POST)) {
      ...
      $sql = "INSERT INTO questoes_online (nome, email, mensagem, datetime_questao) 
        ...
        if($connection->exec($sql)){
          // altera a variável criada no topo do ficheiro para o estado booleano oposto, ou seja, true, isto para poder redireccionar para uma nova página
          $insert_bool = true;
        }
  }
...
<!-- se a variável global $_POST não estiver vazia, quer dizer que foi submetido o formulário e portanto, alterou-se o a variável que serve de switch para o estado TRUE, e portanto, a condição vai-se verificar verdadeira, logo, redirecciona-se o utilizador para outra página com uma mensagem de agradecimento ou algo do género -->
    <?php if ($insert_bool): ?>
        
            <?php header("Location: contacto_enviado.php"); ?>

<!-- se a variável global $_POST estiver vazia, então quer dizer que ainda não foi submetido o formulário e portanto apresenta-se a estrutura de HTML referente a esse mesmo formulário -->
        <?php else: ?>

            <div class="nine columns">

              <dl class="contained tabs">
                <dd class="active"><a href="#contactForm">Contacte a Pseudo Clínica</a></dd>
              </dl>
                    ...
                    <ul class="tabs-content contained">
                      <li id="contactFormTab" class="active">
                          ...
                          <form action="?#m" method="post">
                            ...
                            <div class="two columns"><label class="inline">Nome:</label></div>
                            <div class="ten columns "><input type="text" id="Nome_formulario" placeholder="" name="nome"/>
                            <small id="avisoNome" class="invisivel">Por favor. Introduza um Nome válido...</small>
                            ...
                            <div class="ten columns">
                              <input class="" type="email" id="Email_formulario" placeholder="" name="email"/>
                              <small id="avisoEmail" class="invisivel">Por favor. Introduza um Email válido...</small>
                                        </div>
                                
                                        <label>A sua Questão:</label>
                                        <textarea rows="10" maxlength="1000" id="caixaMensagem" name="questao"></textarea>
                                        
                                
                                        <button type="submit" id="botaoFinaliza" class="radius button" disabled>Enviar</button>
                          </form>
                      </li>
                    </ul>
...
//CAMPO DO NOME
var checar_nome=/^([A-Z,a-z, ,.,ã,á,à,Â,â,ê,í,ú,õ,ó,é,ü,-]){3,100}$/;

Nome.onblur=function() {
  if(!checar_nome.test(Nome.value)){
    Nome.className="error";
    document.getElementById('avisoNome').className="error";
  }else{
    Nome.className="";
    document.getElementById('avisoNome').className="invisivel";
  }
};
...
setInterval(function() {
  if(Nome.value.length && manipuladorEmail && caixaMensagem.value.length){
          botaoFinaliza.removeAttribute("disabled");
  }else{
        botaoFinaliza.setAttribute("disabled", "disabled");
  }
},100);
